...
In our setup, we have deployed Nessus Agents on the EC2 instances within the Web and App tiers of the Prod VPC. We conducted a Basic Agent Scan on these instances to assess vulnerabilities. Additionally, a Nessus Scanner was installed on an EC2 instance within the On-prem VPC. Using the Nessus Scanner, we executed a Basic Network Scan and a Host Discovery Scan, specifically targeting the EC2 instances within the Non-Prod VPC. These scans enable us to gain valuable insights into the security status of our infrastructure, allowing us to address any potential vulnerabilities and enhance overall security.
<br/>
